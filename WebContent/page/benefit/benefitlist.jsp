...
<html>
	...
	<body style="padding-bottom:1.2rem;">
		...
		<div class="gy_title ico1">家滴爱老计划</div>
		...
		<c:forEach items="${mapList.jdbLove}" var="list" varStatus="s">
			<div class="gy_nr">
				<a href="toloveList.html?id=${list.id}">
					<img src="${list.picture}"  style="width:25%;height:80px;margin:auto;">
				</a>
				<a href="toloveList.html?id=${list.id}" class="img-txt">${list.pictureNote}</a>
			</div>
			<c:if test="${s.count > 2}">
				<a href="toloveList.html?id=${list.id}" class="img-txt"><P>>>>更多</P></a>
			</c:if>
		</c:forEach>
